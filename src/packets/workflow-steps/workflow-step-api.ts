...
import {Committee} from "../../committees/committee-api";
...
export class WorkflowStepApi {
  ...
  /**
   * Add a workflow step for a particular committee
   * @param params
   */
  addWorkflowStepStanding({ packetId, workflowStepName, committeeId, workflowStepDueDate, workflowStepNote}: {
    /** packet id */
    packetId: number,
    /** name of the workflow step */
    ...
    return new Promise((resolve, reject) => {
      let url  = WORKFLOW_STEP_BASE_URL.replace("{packet_id}", packetId.toString());
      let formData: {"workflow_step[name]": string, "workflow_step[committee_id]": number, "workflow_step[due_date]": string, "workflow_step[note]"?: string} = {
      ...
      this.apiRequest.executeRest({url: url, method: 'POST', form: formData})
        .then(async (response) => {
          //go get the step
          try {
            let step = await this.getWorkflowStep({packetId: packetId, workflowStepId: response.id});
            resolve(step);
          }
          catch (error) {
            reject(error);
   ...
   * @param params
   */
  getWorkflowStep({packetId, workflowStepId}: {
    /** packet id of the workflow step */
    packetId: number,
    /** workflow step id */
    workflowStepId: number
  }): Promise<WorkflowStep> {
    let apiRequest = this.apiRequest;
    return new Promise(function(resolve, reject) {
      let url = WORKFLOW_STEP_URL
        .replace('{packet_id}', packetId.toString())
        .replace('{workflow_step_id}', workflowStepId.toString());
      apiRequest.executeRest({url: url})
        .then(results => {
          let step = WorkflowStepApi.removeWorkflowStepNesting(results);
          resolve(step);
        })
        .catch(error => reject(error));
    })
