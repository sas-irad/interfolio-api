...
const setupConfigUnit = async (config: TestConfig): Promise<TestConfig> => {
  ...
  if (config.unit) {
    const update = await prompts({
      type: 'select',
      name: 'update',
      message: 'The test unit configuration already exists (' + config.unit.name + ')  Would you like to overwrite?',
      choices: [
  ...
  if (config.apiConfig === undefined)
    throw Error('To run Unit test setup the apiConfig (keys & urls) must first be defined');
